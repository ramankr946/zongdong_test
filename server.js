...
function normTags(t) { if (!t) return ''; if (Array.isArray(t)) return t.join(','); return String(t); }

// highlights must be a real JS array for BigQuery REPEATED STRING column
function normHL(h) {
      if (!h || h === '[]') return [];
...
function bqErrMsg(e) {
      if (e && e.errors && e.errors.length) {
              return e.errors.map(er => {
                        if (er.errors) return er.errors.map(x => x.message || x.reason || JSON.stringify(x)).join('; ');
                        return er.message || er.reason || JSON.stringify(er);
              }).join(' | ');
      }
...
app.get('/products', auth, async (req, res) => {
      try {
              ...
              res.json(rows);
      } catch (e) { res.status(500).json({ error: bqErrMsg(e) }); }
});

app.post('/products', auth, async (req, res) => {
      try {
              const p = req.body;
              if (!p.name || !p.asin || !p.price_inr)
                        return res.status(400).json({ error: 'name, asin and price_inr required' });
              const row = {
                        ...
                        highlights: normHL(p.highlights),
              ...
              const msg = bqErrMsg(e);
              console.error('POST /products:', msg, JSON.stringify(e.errors||null));
              res.status(500).json({ error: msg, details: e.errors||null });
      }
});
...
app.put('/products/:id', auth, async (req, res) => {
      try {
              ...
              const allowed = ['name','brand','asin','category','price_inr','original_price_inr','weight_grams','stock_status','emoji','image_url','description','tags','custom_price_aed'];
              ...
              if (p.highlights !== undefined) {
                        fields.push('highlights = @highlights');
                        params.highlights = normHL(p.highlights);
              }
...
app.post('/orders', auth, async (req, res) => {
      try {
              const o = req.body;
              if (!o.customer_name || !o.product_name || !o.product_asin)
                        return res.status(400).json({ error: 'customer_name, product_name, product_asin required' });
              const row = {
      ...
      try {
              ...
              if (custom_price_aed !== null && custom_price_aed !== undefined && custom_price_aed !== '') {
                        await bq.dataset(DATASET).table('pricing_overrides').insert([{
                                    ...
                                    override_reason: String(override_reason||''),
                                    set_by: 'admin',
              ...
              const msg = bqErrMsg(e);
              console.error('POST /pricing:', msg);
              res.status(500).json({ error: msg });
      }
});
